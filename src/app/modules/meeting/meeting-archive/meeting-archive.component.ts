...
export class MeetingArchiveComponent implements OnInit,OnDestroy {
  ...
  constructor(private router: Router, private dataService: MeetingDataService, private SharedDataService: SharedDataService, private BreakpointObserver: BreakpointObserver) { }
  ...
  setSubscriptions():void {
    ...
    this.getMeetingHistorySubscription = this.dataService.getMeetingHistoryData.subscribe(data=>{
    ...
    this.dataSubscription.push(this.BreakpointObserver.observe(['(min-width:992px)']).subscribe(result=>{
      if(result.matches){
  ...
  deleteMeeting(element):void {
    ...
    this.dataSubscription.push(this.dataService.deleteMeetingHistory(element.meetingHistoryId).pipe(take(1)).subscribe((data:any)=>{
      if(data){
  ...
  expandMeeting(i:number):void {
    if(!this.meetingHistoryData[i].display || this.meetingHistoryData[i].display=='collapsed'){
  ...
  setActiveUserMeetingRole():void {
    for (const m in this.meetingHistoryData){
      for (const mh in this.meetingHistoryData[m].meetingHistory){
        for (const muh in this.meetingHistoryData[m].meetingHistory[mh].meetingUserHistories){
          if(this.meetingHistoryData[m].meetingHistory[mh].meetingUserHistories[muh].uID==this.uID){
            this.meetingHistoryData[m].meetingHistory[mh].userRole=this.meetingHistoryData[m].meetingHistory[mh].meetingUserHistories[muh].userRole
          }
        }
      }
    }
